...
# Dividir las combinaciones en dos partes (segunda mitad)
half_combinations = len(df_colors) // 2
df_colors_user_3_4 = df_colors[half_combinations:]
...
app.layout = html.Div([
    html.H1("Evaluador de combinaciones de colores (Usuarios 3 y 4)", style={'textAlign': 'center'}),
...
def update_colors(yes_clicks, no_clicks):
    ...
    if not ctx.triggered:
        return create_color_display(df_colors_user_3_4.iloc[current_index['value']]), f"{current_index['value'] + 1} de {len(df_colors_user_3_4)}", ""

    # Registrar respuesta
    button_id = ctx.triggered[0]['prop_id'].split('.')[0]
    if button_id == 'yes-button':
        register_response('yes', 'User 3 or 4')  # Asumimos que el usuario es 3 o 4
    elif button_id == 'no-button':
        register_response('no', 'User 3 or 4')

    # Avanzar al siguiente índice
    current_index['value'] = (current_index['value'] + 1) % len(df_colors_user_3_4)
    ...
    return create_color_display(df_colors_user_3_4.iloc[current_index['value']]), f"{current_index['value'] + 1} de {len(df_colors_user_3_4)}", end_message
...
def save_results(n_clicks):
    ...
    if n_clicks > 0:
        ...
        df_responses.to_csv('color_combinations_responses_user_3_4.csv', index=False)
