...
# Asegúrate de reemplazar esto con tu propio DataFrame cargado
...
df_responses = pd.DataFrame(columns=['color1', 'color2', 'combination'])

# Inicializar la app Dash
app = dash.Dash(__name__)

app.layout = html.Div([
    html.H1("Evaluador de combinaciones de colores", style={'textAlign': 'center'}),
...
def update_colors(yes_clicks, no_clicks):
    ...
    if not ctx.triggered:
        return create_color_display(df_colors.iloc[current_index['value']]), f"{current_index['value'] + 1} de {len(df_colors)}", ""

    # Registrar respuesta
    button_id = ctx.triggered[0]['prop_id'].split('.')[0]
    if button_id == 'yes-button':
        register_response('yes')
    elif button_id == 'no-button':
        register_response('no')

    # Avanzar al siguiente índice
    current_index['value'] = (current_index['value'] + 1) % len(df_colors)
    ...
    return create_color_display(df_colors.iloc[current_index['value']]), f"{current_index['value'] + 1} de {len(df_colors)}", end_message
...
def register_response(combination):
    """Registra la respuesta en el DataFrame."""
    global df_responses
    row = df_colors.iloc[current_index['value']]
    new_entry = {'color1': row['color1'], 'color2': row['color2'], 'combination': combination}
...
def save_results(n_clicks):
    ...
    if n_clicks > 0:
        ...
        df_responses.to_csv('color_combinations_responses.csv', index=False)
